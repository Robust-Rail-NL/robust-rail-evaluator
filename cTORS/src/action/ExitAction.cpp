...
void ExitActionGenerator::Generate(const State *state, list<const Action *> &out) const
{
	...
	for (auto ou : outgoing)
		minIndex = (ou->GetStandingIndex() < minIndex) ? ou->GetStandingIndex() : minIndex;
	for (auto ou : outgoing)
	{
		if (ou->IsInstanding())
		{
		...
		for (const auto &[su, suState] : state->GetShuntingUnitStates())
		{
			if (suState.HasActiveAction())
				continue;
			if (su->GetNumberOfTrains() != ou->GetShuntingUnit()->GetNumberOfTrains())
				continue;
			if (suState.position != ou->GetParkingTrack())
				continue;
