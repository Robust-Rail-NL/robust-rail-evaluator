...
pair<bool, string> blocked_track_rule::IsValid(const State* state, const Action* action) const {
	...
	// track. This should be represented as a separate Setback action instead of being
	// embedded in one Move - see doc/known-issue-plan-type.md. A plan declaring
	// schemaVersion >= SETBACK_REQUIRED_SCHEMA_VERSION is rejected outright for this; an
	...
	for ( size_t i = 1; i + 1 < ress.size(); ++i ) {
		...
		if ( ress.at(i-1) == ress.at(i+1) ) {
			...
			if ( state->GetPlanSchemaVersion() >= SETBACK_REQUIRED_SCHEMA_VERSION ) {
				return make_pair(false, "Move action embeds an in-place reversal (saw) at position " + to_string(i) +
					", track " + res->toString() + ", id " + res->GetID() + ", with no explicit Setback action. "
					"Plans declaring schemaVersion " + to_string(SETBACK_REQUIRED_SCHEMA_VERSION) + " or later must "
					"use an explicit Setback action for a reversal - see doc/known-issue-plan-type.md.");
			}
			cerr << "WARNING: Move action embeds an in-place reversal (saw) at position " << i
				 << ", track " << res->toString() << ", id " << res->GetID() << ", with no explicit "
				 << "Setback action. This plan shape is deprecated and will be rejected for any plan "
				 << "declaring schemaVersion " << SETBACK_REQUIRED_SCHEMA_VERSION << " or later - see "
