#!/usr/bin/env bash
# Build and push the multi-arch TORS evaluator image to ghcr.io under the
...
# is its own prerelease identifier, not one chained onto another. The
# date+sha are semver build metadata (after the +), so they never affect
# version precedence/sorting — this is for a human or a bug report to trace a
# running image back to an exact commit and day, not for tooling to compare
# against.
#
...
#
# No -assert or :devel counterpart, unlike docker-push.sh's release images —
